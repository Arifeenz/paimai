...
const AiTripPlanner = ({ onBack }: AiTripPlannerProps) => {
  ...
  const travelStyles = [
    ...
    { value: 'relaxation', label: language === 'th' ? 'พักผ่อน' : 'Relaxation' },
  ...
  const budgetRanges = [
    { value: 'budget', label: language === 'th' ? 'ประหยัด (0-2,000 บาท/วัน)' : 'Budget (0-2,000 THB/day)' },
    { value: 'moderate', label: language === 'th' ? 'ปานกลาง (2,000-5,000 บาท/วัน)' : 'Moderate (2,000-5,000 THB/day)' },
    { value: 'luxury', label: language === 'th' ? 'หรูหรา (5,000+ บาท/วัน)' : 'Luxury (5,000+ THB/day)' }
  ];
      ...
      <Card className="travel-card">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Travel Style */}
            <div className="space-y-2">
              <Label>{language === 'th' ? 'สไตล์การเดินทาง' : 'Travel Style'}</Label>
              <Select value={formData.travelStyle} onValueChange={(value) => setFormData(prev => ({ ...prev, travelStyle: value }))}>
                <SelectTrigger>
                  <SelectValue placeholder={language === 'th' ? 'เลือกสไตล์การเดินทาง' : 'Select travel style'} />
                </SelectTrigger>
                <SelectContent>
            ...
            {/* Budget */}
            <div className="space-y-2">
              <Label>{language === 'th' ? 'งบประมาณ (ต่อคน)' : 'Budget (per person)'}</Label>
              <Select value={formData.budget} onValueChange={(value) => setFormData(prev => ({ ...prev, budget: value }))}>
                <SelectTrigger>
                  <SelectValue placeholder={language === 'th' ? 'เลือกงบประมาณ' : 'Select budget range'} />
                </SelectTrigger>
                <SelectContent>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    {language === 'th' ? 'สร้างแผนการเดินทาง' : 'Generate Trip Plan'}
                  </>
                )}
              </Button>
            </div>
